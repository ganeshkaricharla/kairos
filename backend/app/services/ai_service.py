...
async def goal_setup_opening(
    title: str, description: str, target_date: str | None = None
) -> CoachingReply:
    ...
    raw = await _call_openrouter(goal_analysis.SYSTEM_PROMPT, user_prompt)
    data = json.loads(raw)
...
async def evaluate_progress(
    ...
    raw = await _call_openrouter(progress_evaluation.SYSTEM_PROMPT, user_prompt)
    data = json.loads(raw)
...
async def coaching_reply_ai(
    ...
    raw = await _call_openrouter(coaching_reply.SYSTEM_PROMPT, user_prompt)
    data = json.loads(raw)
    return CoachingReply(**data)
